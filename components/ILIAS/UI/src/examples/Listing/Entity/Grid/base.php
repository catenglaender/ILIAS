        ...
        public function map(UIFactory $ui_factory, mixed $record): Entity
        {
            ...
            $entity = $ui_factory->entity()->standard(
                ...
                ->withFeaturedProperties(
                    $ui_factory->listing()->property()
                        ->withProperty("Creator", $creator, true, $ui_factory->symbol()->glyph()->user())
                )
                ->withManagingActions(...$managing_actions)
                ->withMainDetails(
                    $ui_factory->listing()->property()
                        ->withProperty("Description", $description, false)
                        ->withProperty("Duration", $duration, true, $ui_factory->symbol()->glyph()->time())
                        ->withProperty("Upload Date", $date, true, $ui_factory->symbol()->glyph()->calendar())
                )
            ...
            if($availability) {
                $entity = $entity->withBlockingAvailabilityConditions(
                    $ui_factory->listing()->property()
                        ->withProperty("Status", $availability, true, $ui_factory->symbol()->glyph()->eyeclosed())
                );
            }
            if($add_workflow) {
